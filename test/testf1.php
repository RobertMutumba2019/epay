...
if ($response === false) {
    ...
    if (isset($responseData['access']) && isset($responseData['refresh'])) {
        ...
        $forexJson = json_encode($forexData);
        ...
        $forexOptions = array(
            'http' => array(
                ...
                'header' => "Content-Type: application/json\r\n" .
                            ...
                            "Content-Length: " . strlen($forexJson) . "\r\n",
                'content' => $forexJson,
                'ignore_errors' => true
            )
        );

        $forexContext = stream_context_create($forexOptions);
        $forexResponse = file_get_contents($forexUrl, false, $forexContext);

        if ($forexResponse === false) {
            echo "Error: Failed to connect to Forex Rates API.";
        } else {
            echo "\nForex Rates Response:\n";
            print_r(json_decode($forexResponse, true));
        }
        // ===============================================================
